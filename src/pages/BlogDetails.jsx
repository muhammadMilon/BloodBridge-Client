...
export default function BlogDetails() {
  ...
  const staticBlogs = [
    ...
    },
    {
      ...
      thumbnail:
        "https://images.pexels.com/photos/6823566/pexels-photo-6823566.jpeg",
      content:
  ...
  useEffect(() => {
    ...
    const staticBlog = staticBlogs.find(b => b._id === ID);
    if (staticBlog) {
        setDetails(staticBlog);
        setLoading(false);
        return;
    }

    // Otherwise fetch from API
    axiosSecure.get(`/blog-details/${ID}`)
      .then((res) => {
        setDetails(res.data);
        setLoading(false);
      })
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 relative z-10">
        ...
        <div className="max-w-4xl mx-auto">
          <img
            src={thumbnail}
            alt={title}
            className="w-full h-64 sm:h-96 object-cover rounded-2xl shadow-2xl border border-slate-800 mb-8"
          />
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black mb-6 text-white leading-tight">
            {title}
          </h1>
          <div className="text-sm text-slate-400 mb-8 flex flex-wrap gap-4 justify-between items-center border-b border-slate-800 pb-6">
             <span className="px-3 py-1 bg-emerald-900/30 text-emerald-400 border border-emerald-900/50 rounded-full font-bold text-xs uppercase tracking-wider">
              {status}
            </span>
            <span className="font-semibold">{formattedDate}</span>
          </div>
          <div
            className="prose prose-lg sm:prose-xl max-w-full prose-invert prose-headings:font-bold prose-headings:text-gray-100 prose-p:text-slate-400 prose-a:text-emerald-400 hover:prose-a:text-emerald-300 prose-strong:text-white prose-li:text-slate-400 leading-relaxed"
